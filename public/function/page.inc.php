...
$page：分页的get参数
*/
function page($style,$count,$page_size,$num_btn=10,$page='page'){
	...
	if($num_btn>=$page_num_all){
		...
		for($i=1;$i<=$page_num_all;$i++){//这边的$page_num_all是限制循环次数以控制显示按钮数目的变量,$i是记录页码号
			if($_GET[$page]==$i){
				$html[$i]="<button class=\"{$style}\"><span>{$i}</span></button>";
			}else{
				$html[$i]="<button class=\"{$style}\"><a style=\"color:white\" href='{$url}{$i}'>{$i}</a></button>";
			}
		}
	}else{
		...
		for($i=0;$i<$num_btn;$i++){
			if($_GET[$page]==$start){
				$html[$start]="<button class=\"{$style}\"><span>{$start}</span></button>";
			}else{
				$html[$start]="<button class=\"{$style}\"><a style=\"color:white\" href='{$url}{$start}'>{$start}</a></button>";
			}
			$start++;
		}
		...
		if(count($html)>=3){
			...
			if($key_first!=1){
				array_shift($html);
				array_unshift($html,"<button class=\"{$style}\"><a style=\"color:white\" href='{$url}=1'>1...</a></button>");
			}
			if($key_end!=$page_num_all){
				array_pop($html);
				array_push($html,"<button class=\"{$style}\"><a style=\"color:white\" href='{$url}={$page_num_all}'>...{$page_num_all}</a></button>");
			}
		}
	}
	if($_GET[$page]!=1){
		$prev=$_GET[$page]-1;
		array_unshift($html,"<button class=\"{$style}\"><a style=\"color:white\" href='{$url}{$prev}'> 上一页</a></button>");
	}
	if($_GET[$page]!=$page_num_all){
		$next=$_GET[$page]+1;
		array_push($html,"<button class=\"{$style}\"><a style=\"color:white\" href='{$url}{$next}'>下一页 </a></button>");
	}
	$html=implode(' ',$html);
	$data=array(
			'limit'=>$limit,
